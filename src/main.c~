...
void WriteFileInInode (char* buff, int pos_inode){
	int size = StrToBlocks (buff);//нет переключения флага на занятость этого inode
	int is_file = 1;
...
int FindNumberCatalogInCatalogInode(int pos_inode, char* name){//name catalog
	if (ReadBlockLInt (pos_inode + block_size) == 1){
...
void DeleteCatalogInCatalogInode(int pos_inode, char * name){ //name catalog
	int number_catalog = FindNumberCatalogInCatalogInode(pos_inode, name);
	int size = ReadBlockLInt (pos_inode);
...
void Cat(int pos_catalog_inode, char* name){//name without .txt
	int number = FindNumberCatalogInCatalogInode(pos_catalog_inode, name);
	if (number == -1){
...
void init(){
	...
	fprintf(save_file,"%02d", 0);//sizeI
	fprintf(save_file,"%01000d", 0);//sizeB
	fprintf(save_file,"%0512d", 0);//sizeI*inode_size
    fclose(save_file); 
...
int main()
{
	...
	while(fgets (buff, size, stdin) != NULL) {
        ...
        if (strcmp(command, "touch") == 0) {
			char* name = GetCommand(buff + strlen(command) + 1);
			AddFileInCatalog (pos_catalog_inode, name, buff + strlen(command)
			                  + 1 + strlen(name) + 1);
			free(name);
			...
			char* catalog = ReadBlockInInode (pos_catalog_inode, 1);
			...
			char* name = GetArg(buff + strlen(command) + 1, '.');
